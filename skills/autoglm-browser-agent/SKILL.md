...
> **🚨 最重要的规则 2(首次对话):执行任何浏览器任务之前,必须先读取 `~/.openclaw-autoclaw/config.json` 检查 `browser` 字段。如果 `browser` 字段不存在,必须先询问用户"你使用的是哪个浏览器？Chrome / Edge",等用户回答并写入配置后,才能执行任务。禁止跳过此步骤。**
...
### Step 1: 解除 macOS 签名限制（仅 macOS，首次需要，已执行过自动跳过）
...
如果上述命令未返回 server 信息，**必须立即执行以下注册命令**（不要让用户去看 INSTALL.md）：
...
### Step 4: 安装浏览器扩展（如遇到扩展连接超时才需要）

如果执行任务时遇到 `扩展连接超时` / `Failed to initialize browser` 错误，说明浏览器扩展未安装或未启用，**必须引导用户安装**：

- Chrome / Brave / Arc：[AutoGLM 扩展（Chrome Web Store）](https://chromewebstore.google.com/detail/autoglm/jelniggicmclhfgnlapbkgfibmgelfnp?hl=zh-CN&utm_source=ext_sidebar)
...
> ✅ **正确写法**(单行,task 原文照抄):
> ```
> {baseDir}/dependency/mcporter call autoglm-browser-agent.browser_subagent task="打开微博搜索 pgone" start_url="https://weibo.com" --timeout 7200000
> ```
...
### 1.5 Check Browser Preference & Trust Mode(首次对话必检)

**每次对话的第一次调用 `browser_subagent` 之前**,读取 `~/.openclaw-autoclaw/config.json`,依次检查:
...
> **⚠️ 以上两项如有缺失,必须在同一轮询问中一起问完（不要分两轮）,等用户全部回答后再执行任务。**
...
```bash
{baseDir}/dependency/mcporter call autoglm-browser-agent.browser_subagent task="用户交互操作完成确认说明。<原始/剩余任务>" session_id="<session_id>" --timeout 7200000
```
...
{baseDir}/dependency/mcporter call autoglm-browser-agent.browser_subagent task="用户敏感操作同意与否说明。<原始/剩余任务>" session_id="<session_id>" --timeout 7200000
```
...
1. **Check setup first** — `{baseDir}/dependency/mcporter list` 找不到 server 时,参阅 `{baseDir}/INSTALL.md`
